...
class ATM:
    ...
    def __init__(self, root):
        ...
        self.root.configure(bg="dodgerblue2")
        ...
        self.create_user_button = self.create_button("CREATE USER", self.create_user)
    ...
    def create_user(self):
        ...
        new_username = simpledialog.askstring("Create User", "Enter a new username:")
        new_pin = simpledialog.askstring("Create User", "Enter a 4-digit PIN for the new user:", show="*")
        
        if new_username and len(new_username) > 0 and new_pin and len(new_pin) == 4:
            
            try:
                
                self.cur.execute("INSERT INTO users (username, balance, pin) VALUES (%s, 0.00, %s)", (new_username, new_pin))
                self.conn.commit()
